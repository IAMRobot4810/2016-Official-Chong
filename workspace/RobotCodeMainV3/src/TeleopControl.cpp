...
void TeleopControl::ButtonControl(){
	...
	if(sensorOverride == false){
		...
		if(a2Button->Get() && !b2Button->Get()&& r2StickIn < 0.1 && r2StickIn > -0.1){
			shoot->LowerNoSense(0.5); //LowerNoSense(0.25)
		}

		else if(b2Button->Get() && !a2Button->Get() && r2StickIn < 0.1 && r2StickIn > -0.1){
			shoot->RaiseNoSense(0.5); //RaiseNoSense(0.25)
		}

		else if(!b2Button->Get() && !a2Button->Get() && r2StickIn >= 0.1){
			shoot->LowerNoSense(r2StickIn);
		}

		else if(!b2Button->Get() && !a2Button->Get() && r2StickIn <= -0.1){
			shoot->RaiseNoSense(-r2StickIn);
		}
