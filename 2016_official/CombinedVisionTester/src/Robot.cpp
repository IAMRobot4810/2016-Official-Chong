...
private:
	...
	void RobotInit()
	{
		...
		cameraModeToggle = true; //0 for standard, 1 for retro
		buttonLock = false;
	}
	...
	void TeleopPeriodic()
	{
		...
		if(button->Get() && !buttonLock){
			buttonLock = true;
			if(cameraModeToggle){
				if(error != IMAQdxErrorSuccess) {
					DriverStation::ReportError("IMAQdxGrab error: " + std::to_string((long)error) + "\n");
				} else {
					//imaqDrawShapeOnImage(frame, frame, { 100, 100, 10, 50 }, DrawMode::IMAQ_DRAW_VALUE, ShapeMode::IMAQ_SHAPE_RECT, 0.0f);
					CameraServer::GetInstance()->SetImage(process.frame);
				}
			}else{
				VisionRunner();
			}
